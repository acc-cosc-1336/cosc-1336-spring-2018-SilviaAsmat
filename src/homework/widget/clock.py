...
class Clock(object):
    ...
    def __init__(self, tk, zone=None, **kargs):
        ...
        self.canvas = tkinter.Canvas(
            self.internal_frame, width=self.width, height=250)
        self.canvas.grid()
    ...
    def draw_minute_hand(self):
        ...
        if time_from_utc(0, self.diff) % 1 == 0.5:
            minutes_angle += 180
        elif time_from_utc(0, self.diff) % 1 == 0.75:
           minutes_angle += 270
    ...
    def draw_time_text(self):
        #i've written the code for calling get_time DON'T CHANGE THIS CODE
        self.canvas.create_text(95, 225, text=get_time(int(self.hour), int(self.minutes),
